# Diff: Using faster way to get ground truth collision detection
...
def main():
    ...
    robot_ids = [collision_bodies[body] for body in collision_bodies if "robot" in body]
    ...
    for i in tqdm(range(0, args.num_samples)):
        ...
        # new joint state
        fk_start = time.time()
        for robot_num in range(len(robot_ids)):
            robot_id = robot_ids[robot_num]
            for j in range(pyb.getNumJoints(robot_id, physicsClientId=sim_id)):
                pyb.resetJointState(robot_id, j, Q_trial_robot[i][robot_num][j], physicsClientId=sim_id)
        extra_fk_query_time += (time.time() - fk_start)
        
        # check collision
        pyb.performCollisionDetection(physicsClientId=sim_id)
        num_collisions = len(pyb.getContactPoints())
    ...
    print(set([Q_row[-1] for Q_row in Q]))
