...
def train_test_deep_learning(X_train, Y_train, X_test, learning_rate=0.01):
    model = nn.Sequential(
          nn.Linear(14, 100),
          nn.Tanh(),
          nn.Linear(100, 150),
    ...
    for i in range(EPOCHS):
        ...
        print('loss in epoch {}: {}'.format(i, total_loss))
        # calculate validation loss
        x = torch.FloatTensor(X_train[len(Y_train) - BATCH_SIZE:])
        y = torch.FloatTensor([Y_train[len(Y_train) - BATCH_SIZE:]]).reshape((BATCH_SIZE, 1))
